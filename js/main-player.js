/*$(function () {
	...
	$('.de-nav-ul li:nth-child(3)').hover(function() {
	...
	$('.per-playall li').hover(function() {
		$(this).children('a').css('opacity', '1');
		$(this).children('i').css('opacity', '1');
		$(this).css('opacity', '1');
	}, function() {
		$('.per-playall li a').css('opacity', '0.6');
		$('.per-playall li i').css('opacity', '0.6');
		$('.per-playall li').css('opacity', '0.6');
	});
...
$(function() {
        ...
        function parseLyric(text) {
            //按行分割歌词
            let arr = text.split('\n');
            //console.log(arr)//0: "[ti:七里香]" "[ar:周杰伦]"...
            let result = []; //新建一个数组存放最后结果
            //遍历分割后的歌词数组，将格式化后的时间节点，歌词填充到result数组
            for (i = 0; i < arr.length; i++) {
                let playTimeArr = arr[i].match(/\[\d{2}:\d{2}((\.|\:)\d{2})\]/g); //正则匹配播放时间
                let lineLyric = "";
                if (arr[i].split(playTimeArr).length > 0) {
                    lineLyric = arr[i].split(playTimeArr);
                }
